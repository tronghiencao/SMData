...
function EasyCache() {
  ...
  */
  ...
  function clean() {
    ...
    console.log("Removing current cache keys: "+JSON.stringify(keys));
  ...
  function _pushCacheKeys(cache, key, ttl) {
    ...
    if (DEBUG) {
      console.log("PUSHING NEW CACHE KEY: "+key);
      console.log("CURRENT CACHE KEYS: "+JSON.stringify(keys));
    }
